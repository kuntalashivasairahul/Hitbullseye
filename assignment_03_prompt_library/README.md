...
```text
assignment_03_prompt_library/
├── data/
│   └── golden_set.json             # 50 validated benchmark test cases
├── prompts/
│   ├── __init__.py                 # Prompts package entrypoint
│   ├── zero_shot.py                # Strategy v1.0.0 (Minimal baseline)
│   ├── few_shot.py                 # Strategy v1.1.0 (3 customer support demos)
│   ├── chain_of_thought.py         # Strategy v1.2.0 (Step-by-step reasoning)
│   └── structured_template.py      # Strategy v1.3.0 (Rigid Markdown/JSON output)
├── src/
│   ├── __init__.py                 # Library source package exports
│   ├── dataset_generator.py        # Dataset builder & schema validator
│   ├── prompt_registry.py          # Central registry & CLI preview tool
│   ├── evaluator.py                # Automated evaluation rubric & scoring heuristics
│   ├── llm_client.py               # Modular client with deterministic MockLLMBackend
│   └── benchmark_runner.py         # Batch runner (200 evaluations) & metrics aggregator
├── results/
│   ├── benchmark_results.json      # Complete telemetry and evaluation logs
│   ├── summary_table.csv           # Aggregated metrics per prompt strategy
│   └── failure_catalogue.json      # Logs of sub-optimal responses (Score < 4 or Format Fail)
├── tests/
│   ├── __init__.py
│   ├── test_dataset_generator.py   # Golden set & generator unit tests (7 tests)
│   ├── test_prompt_registry.py     # Strategy & registry unit tests (7 tests)
│   └── test_evaluator.py           # Rubric, heuristics & runner tests (11 tests)
